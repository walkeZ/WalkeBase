...
public class VolumeViewDoubleSine extends View {
    ...
    public VolumeViewDoubleSine(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    private void drawSine(Canvas canvas, Path path, Paint paint, int period, float drawWidth, float amplitude) {
        ...
        for (float x = -halfDrawWidth; x <= halfDrawWidth; x++) {
            y = (float) sine(x, period, drawWidth) * amplitude * (1); 
            path.lineTo(x, y);
        }
