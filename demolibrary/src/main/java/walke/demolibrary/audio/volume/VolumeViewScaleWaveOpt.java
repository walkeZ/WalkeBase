...
public class VolumeViewScaleWaveOpt extends View {
    ...
    private static final float AMPLITUDE_SIZE_SCALE = 6f;

    public VolumeViewScaleWaveOpt(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        mAmplitude = mDrawHeight / AMPLITUDE_SIZE_SCALE;
    }
    ...
    private void drawSine(Canvas canvas, Path path, Paint paint, int period,
        ...
        for (float x = -halfDrawWidth; x <= halfDrawWidth; x++) {
            ...
            y = (float) ((sine(x, period, drawWidth) + SINE_UP_AND_DOWN_MOVE) * amplitude * (1) * Math
                    .pow(scaling, 3));
            path.lineTo(x, y);
        }
