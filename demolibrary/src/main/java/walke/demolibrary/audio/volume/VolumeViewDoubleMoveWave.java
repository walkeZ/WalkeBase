...
public class VolumeViewDoubleMoveWave extends View {
    ...
    private static final int SMALL_WAVE_PERIOD = 3;
    
    private static final int BIG_WAVE_PERIOD = 3;
    
    private float mMaxBigWaveAmplitude;
    
    private float mMaxSmallWaveAmplitude;

    public VolumeViewDoubleMoveWave(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        mMaxSmallWaveAmplitude = mDrawHeight / 4f;
        mMaxBigWaveAmplitude = mDrawHeight / 2.5f;
    }
    ...
    private void drawWave(Canvas canvas, Path path, Paint paint, int period,
        ...
        drawSine(canvas, mPath, mPaint, BIG_WAVE_PERIOD, mDrawWidth, mMaxBigWaveAmplitude, mPhase * BIG_WAVE_SPEED);
        paint.setColor(SMALL_WAVE_COLOR);
        drawSine(canvas, mPath, mPaint, SMALL_WAVE_PERIOD, mDrawWidth, mMaxSmallWaveAmplitude, mPhase * SMALL_WAVE_SPEED);
