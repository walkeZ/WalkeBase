...
public class BasePermissionActivity extends AppCompatActivity {
    ...
    private static final String[] requestPermissions = {PERMISSION_ACCESS_RECORD_AUDIO, PERMISSION_ACCESS_WRITE_EXTERNAL_STORAGE, PERMISSION_ACCESS_READ_EXTERNAL_STORAGE,};
...
//        }
        ...
        try {
            ...
            Toast.makeText(activity, "please open this permission:" + requestPermission, Toast.LENGTH_SHORT).show();
    ...
    public void requestMultiPermissions(final Activity activity, PermissionsGrant grant) {
        ...
        if (permissionsList.size() > 0) {
            ActivityCompat.requestPermissions(this, permissionsList.toArray(new String[permissionsList.size()]), CODE_MULTI_PERMISSION);
        } else if (shouldRationalePermissionsList.size() > 0) {
            showMessageOKCancel(activity, "should open those permission", new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialog, int which) {
                    ActivityCompat.requestPermissions(getParent(), shouldRationalePermissionsList.toArray(new String[shouldRationalePermissionsList.size()]), CODE_MULTI_PERMISSION);
                }
            });
        } else {
    ...
    private void requestMultiResult(Activity activity, String[] permissions, int[] grantResults, PermissionsGrant permissionGrant) {
        ...
        if (notGranted.size() == 0) {
            Toast.makeText(activity, TAG + "all permission success" + notGranted, Toast.LENGTH_SHORT).show();
    ...
    public void requestPermissionsResult(final Activity activity, final int requestCode, @NonNull String[] permissions,
        ...
        LogUtils.v(TAG + "requestPermissionsResult requestCode:" + requestCode + ",permissions:" + permissions[0] + ",grantResults:" + grantResults[0] + ",length:" + grantResults.length);
