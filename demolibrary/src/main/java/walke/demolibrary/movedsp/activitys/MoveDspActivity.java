...
import walke.demolibrary.movedsp.components.URecorder;
 ...
 * https://blog.csdn.net/sctu_vroy/article/details/45871823
 */
public class MoveDspActivity extends AppCompatActivity implements OnClickListener {
    ...
    private VisualizerWaveView mWaveView = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_move_dsp);
        mContext = getApplicationContext();
        application = (CustomApplication) getApplication();

        width = WindowUtil.getWindowWidth(this);
        height = WindowUtil.getWindowHeight(this) / 2;
    ...
    public void onClick(View v) {
        ...
        } else if (id == R.id.btnMusic_play) {//傅里叶变换（边播放边进行）
...
//				tvTips.setText("播放中……");
            ...
            if (isfinish) {
                isfinish = false;
//                Uri uri = null;
//                if (isAudio) uri = Uri.parse(AudioPath);
//                else uri = Uri.parse(Mp3Path);
                mMedia = MediaPlayer.create(this, R.raw.aaaass);
                duration = mMedia.getDuration();
                progress.setMax(duration / 1000);
                if (duration / 1000 % 60 < 10)
                    tvMediaAll.setText(duration / 1000 / 60 + ":0" + duration / 1000 % 60);
                else tvMediaAll.setText(duration / 1000 / 60 + ":" + duration / 1000 % 60);
            }
            setupVisualizerFxAndUI();
            mVisualizer.setEnabled(true);
            mMedia.start();
            mHandler.post(updateThread);
        } else if (id == R.id.btnMusic_stop) {//停止播放，停止傅里叶变换
...
//				tvTips.setText("停止播放……");
            ...
            saveBufData();
        } else if (id == R.id.btnMusic_pause) {//暂停播放，暂停傅立叶变换
            mHandler.removeCallbacks(updateThread);
            isfinish = false;
//				tvTips.setText("暂停中……");
            mMedia.pause();
            mVisualizer.setEnabled(false);
        } else if (id == R.id.btn_LowPass) {//低通滤波
        } else if (id == R.id.btn_ButterWorth) {//巴特沃斯滤波
        } else if (id == R.id.btn_IIR) {//IIR滤波
        } else if (id == R.id.btn_FIR) {//FIR滤波
        } else if (id == R.id.btnRecord_begin) {//开始录音
            tvTips.setText("录音中……");
            mRecorder.start();
            Toast.makeText(MoveDspActivity.this, "开始录音", Toast.LENGTH_SHORT).show();
        } else if (id == R.id.btnRecord_stop) {//结束录音
            tvTips.setVisibility(View.VISIBLE);
            mRecorder.stop();
            tvTips.setText("record.amr");
            Toast.makeText(MoveDspActivity.this, "结束录音", Toast.LENGTH_SHORT).show();
...
//		bufFFt = new ArrayList<byte[]>();
    ...
    private void setupVisualizerFxAndUI() {
        if (isAddVisualizer++ == 0) {
            LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT, (int) (VISUALIZER_HEIGHT_DIP * getResources().getDisplayMetrics().density));
            lp.bottomMargin = 5;
            ...
            mWaveView = new VisualizerWaveView(this);
        ...
        mMedia.setOnCompletionListener(mp -> {
            mHandler.removeCallbacks(updateThread);
            progress.setProgress(0);
            tvMediaNow.setText("0:00");
            isfinish = true;
//				tvTips.setText("播放完毕……");
    ...
    public boolean hasSDcard() {
        return true;
//        if (DeviceUtil.isSdcardEnable()) return true;
//        else {
        ...
        public ResPopupView(Context context) {
            ...
            view.setOnTouchListener(new OnTouchListener() {
                ...
                public boolean onTouch(View v, MotionEvent event) {
                    ...
                    if (event.getAction() == MotionEvent.ACTION_UP) if (top < height) dismiss();
                    return true;
                }
            });
        }
