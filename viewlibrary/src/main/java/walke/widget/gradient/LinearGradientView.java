...
public class LinearGradientView extends View {
...

    {
        ...
        mRectRadius = ViewUtil.dpToPx(getContext(), mRectRadius);
        colorThree = new int[]{
        ...
        colorTwo = new int[]{Color.parseColor("#ff0000"),
                Color.parseColor("#0000ff")};
        mSpace = (int) ViewUtil.dpToPx(getContext(), 2);
    ...
    public void drawLinearRound(Canvas canvas) {
        //绘制两色渐变
        int rectHeight = (int) ViewUtil.dpToPx(getContext(), 20);
        int space = 10;
        //水平clamp
        ...
        if (mWidth < 5) {
            ...
            mHeightNum = (int) (getMeasuredHeight() - ViewUtil.dpToPx(getContext(), 120) / mWidth);
        }
        //绘制三色渐变
        int count = 0;
        RectF rectF = new RectF();
        for (int i = 0; i < mHeightNum; i++) {
            rectF.top = i * mWidth + (i + 1) * mSpace + (int) ViewUtil.dpToPx(getContext(), 120);
