...
public class MyWheelView extends View {
     ...
     * 设置数据
     */
    private List<AreaInfo> dataList = new ArrayList<>();
    /**
     * 按下的坐标
     */
    private int downY;
     ...
     * 刷新界面
     */
    private static final int THREAD_SELECTED = 0x002;
    /**
     * 移动距离
     */
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (controlWidth != 0) {
            setMeasuredDimension(getWidth(), (int) ((itemNumber + 0.5f) * unitHeight));
            controlWidth = getWidth();
        }
...
//                    mOnSelectListener.endSelect(item.areaInfo);
                Message tMessage = handler.obtainMessage();
                tMessage.what = THREAD_SELECTED;
                tMessage.obj=item.areaInfo;
    ...
    private void initData() {
        ...
        if (dataList != null) {
            ...
            for (int i = 0; i < dataList.size(); i++) {
                ...
                itmItemObject.index = i;
                itmItemObject.itemText = dataList.get(i).getAreaName();
                itmItemObject.x = 0;
                itmItemObject.y = i * unitHeight;

                itmItemObject.areaInfo=dataList.get(i);
        ...
        if (move > 0) {
            for (int i = 0; i < itemList.size(); i++) {
                if (itemList.get(i).isSelected()) {
                    ...
                    Message tMessage = handler.obtainMessage();
                    tMessage.what = THREAD_SELECTED;
                    tMessage.obj=itemList.get(i).areaInfo;
                    handler.sendMessage(tMessage);
//                    if (mOnSelectListener != null)
//                        mOnSelectListener.endSelect(itemList.get(i).areaInfo);
                    break;
                }
            }
        } else {
    ...
    private synchronized void slowMove(final int move) {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                for (ItemObject item : itemList) {
                    if (item.isSelected()) {
//                        if (mOnSelectListener != null) {//这里是子线程可能发生更改UI操作从而报错
//                            Log.i("walke", "slowMove:----------------------- name="+Thread.currentThread().getName());
//                            mOnSelectListener.endSelect(item.areaInfo);
//                        }
                        Message tMessage = handler.obtainMessage();
                        tMessage.what = THREAD_SELECTED;
    ...
    private void onSelectListener() {
        ...
        for (ItemObject item : itemList) {
            if (item.isSelected()) {
                mOnSelectListener.selecting(item.areaInfo);
            }
        }
    }
   ...
   /* private void drawMask(Canvas canvas) {
        LinearGradient lg = new LinearGradient(0, 0, 0, maskHight, 0x66FF0033,
                0x66FF0033, TileMode.MIRROR);
        Paint paint = new Paint();
        paint.setShader(lg);
        canvas.drawRect(0, 0, controlWidth, maskHight, paint);

        LinearGradient lg2 = new LinearGradient(0, controlHeight - maskHight,
                0, controlHeight, 0x66FF0033, 0x66FF0033, TileMode.MIRROR);
        Paint paint2 = new Paint();
        paint2.setShader(lg2);
        canvas.drawRect(0, controlHeight - maskHight, controlWidth,
                controlHeight, paint2);
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        controlHeight = (itemNumber + 0.5f) * unitHeight;
     ...
     * @param datas
     */
    public void resetData(List<AreaInfo> datas) {
        setData(datas);
        //invalidate();
        postInvalidate();
    }
     ...
     * @param info
     */
    public void setDefaultByInfo(AreaInfo info) {
        if (itemList!=null&&itemList.size()>0){
            for (int i = 0; i < itemList.size(); i++) {
                if (itemList.get(i).areaInfo.getAreaId().equals(info.getAreaId())){
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case THREAD_SELECTED:
                    if (mOnSelectListener != null) {
                        Log.i("walke", "slowMove:----------------------- name="+Thread.currentThread().getName());
                        mOnSelectListener.endSelect((AreaInfo) msg.obj);
                    }
                    break;
                default:
                    break;
    ...
    public interface OnSelectListener {
         ...
         * @param areaInfo
         */
        void endSelect(AreaInfo areaInfo);

        /**
         * 选中的内容
         * @param areaInfo
         */
        void selecting(AreaInfo areaInfo);
